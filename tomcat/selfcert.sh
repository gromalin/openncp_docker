...
# Certificates alias
CAcertAlias=pfr.ca.epsos.$organization.$country
NcpSignatureAlias=pfr.ncp-signature.epsos.$organization.$country
VPNServerSignatureAlias=pfr.ncp-vpns.epsos.$organization.$country
VPNClientSignatureAlias=pfr.ncp-vpnc.epsos.$organization.$country
ServiceConsumerSignatureAlias=pfr.ncp-sc.epsos.$organization.$country
ServiceProviderSignatureAlias=pfr.ncp-sp.epsos.$organization.$country
OCSPSignatureAlias=pfr.ncp-ocsp.epsos.$organization.$country
...
keytool -noprompt -importcert -alias $CAcertAlias -file ROOT/$country-ca.pem -keystore keystore/$country-signature-keystore.jks -storepass $passwordKS
keytool -noprompt -importcert -alias $CAcertAlias -file ROOT/$country-ca.pem -keystore keystore/$country-service-consumer-keystore.jks -storepass $passwordKS
keytool -noprompt -importcert -alias $CAcertAlias -file ROOT/$country-ca.pem -keystore keystore/$country-service-provider-keystore.jks -storepass $passwordKS
keytool -noprompt -importcert -alias $CAcertAlias -file ROOT/$country-ca.pem -keystore keystore/$country-ocsp-keystore.jks -storepass $passwordKS
...
if [ "$initialTrustStore" ]; then
  ...
  keytool -noprompt -importcert -alias $CAcertAlias -file ROOT/$country-ca.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
fi

keytool -noprompt -importcert -alias $NcpSignatureAlias -file pem/$country-ncp-sig-self-sign.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
keytool -noprompt -importcert -alias $ServiceConsumerSignatureAlias -file pem/$country-ncp-sc-self-sign.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
keytool -noprompt -importcert -alias $ServiceProviderSignatureAlias -file pem/$country-ncp-sp-self-sign.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
keytool -noprompt -importcert -alias $VPNServerSignatureAlias -file pem/$country-ncp-vpn-server-self-sign.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
keytool -noprompt -importcert -alias $VPNClientSignatureAlias -file pem/$country-ncp-vpn-client-self-sign.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
keytool -noprompt -importcert -alias $OCSPSignatureAlias -file pem/$country-ncp-ocsp-self-sign.pem -keystore keystore/$country-truststore.jks -storepass $passwordTS
